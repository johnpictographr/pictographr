...
class Base {
	 ...
	 */
	protected function set_options(array $override_options = NULL) {
	  $options = array(
	    // Obtain your Client ID and secret from your Clever developer dashboard at https://account.clever.com/partner/applications
	    'client_id' => '835bc15c45fbddcd15bd',
	    'client_secret' => 'c345301df1a7852b92fc4c8299c38c3bcb4628d2',
	    'clever_redirect_base' => 'https://pictographr.com/partners/clever',
	 ...
	 */
	protected function process_incoming_requests($incoming_request_uri, array $options) {
	  if(preg_match('/oauth/', $incoming_request_uri)) {
	    try {
	    	
	      $me = $this->process_client_redirect($_GET['code'], $options);
	      
	      // xuser_id
	      
	      $begin_url = "https://pictographr.com/auth/router?redirect=newfile&partner_id=8&xorg_id=" . $me['data']['district'] . "&";
	      
				if( $me['type'] == 'district_admin'){
					$url = $begin_url . "org_type=E&setUpOrganization=true&isOrgAdmin=true&popSetupModal=true";
				} else{
					$url = $begin_url . "setUpMember=true";
				};
	    ...
	    header("Location: " . $this->generate_sign_in_with_clever_url($options));
	    
	    return;
	    
	    $sign_in_link =  $this->generate_sign_in_with_clever_link($options);
	 ...
	 */
	protected function process_client_redirect($code, array $options) {
	  $bearer_token = $this->exchange_code_for_bearer_token($code, $options);
	  $request_options = array('method' => 'GET', 'bearer_token' => $bearer_token);
	  $me_response = $this->retrieve_me_response_for_bearer_token($bearer_token, $options);
	 ...
	 */
	protected function exchange_code_for_bearer_token($code, array $options) {
	  $data = array('code' => $code,
	                'grant_type' => 'authorization_code',
	                'redirect_uri' => $options['client_redirect_url']);
	  $request_options = array('method' => 'POST', 'data' => $data);
	  $response = $this->request_from_clever($options['clever_oauth_tokens_url'], $request_options, $options);
	 ...
	 */
	protected function retrieve_me_response_for_bearer_token($bearer_token, array $options) {
	  $request_options = array('method' => 'GET', 'bearer_token' => $bearer_token);
	  $response = $this->request_from_clever($options['clever_api_me_url'], $request_options, $options);
	  // Evaluate if the response is successful
	  if ($response && $response['response_code'] && $response['response_code'] == '200') {
	 ...
	 */
	protected function request_from_clever($url, array $request_options, array $clever_options) {
	  $ch = curl_init($url);
	 ...
	 */
	protected function generate_sign_in_with_clever_url(array $options) {
	...
	*/
	protected function generate_sign_in_with_clever_link(array $options) {
	  $html = "<a href='" . $this->generate_sign_in_with_clever_url($options) . "'><img src='http://assets.clever.com/sign-in-with-clever/sign-in-with-clever-small.png'/></a>";
	  return $html;
	}
