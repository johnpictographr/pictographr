$(document).ready(function() {
    ...
    "thumbnailLink": "https://lh4.googleusercontent.com/DoDXcCfG5ig99Rc6BNz3TPuBdRaJ0-6sGPI8BhOf--Kl87pDxkp8OftRVWKcuh_Or2WKqw=s220"
  },
  {
    "fileId": "0B5ptY5tUIebjcENSYi1pRUlubUk",
    "thumbnailLink": "https://lh5.googleusercontent.com/UkxoyDruDjZlVLwdK2X91BnsH83BZHBZU0-qDD-1p8BFzyA3Qn4wJ4QTxUBCgS7JxUGWNw=s220"
  },
  {
    "fileId": "0B5ptY5tUIebjMy1PR3BaUklJTzg",
    "thumbnailLink": "https://lh4.googleusercontent.com/vVa6LcnC6KRPwS-0J23W1wkC5YX_J-CqhChVLAY25SmsADQkEcsoBUDl53ssnQ0INqlG7Q=s220"
  },
  {
    "fileId": "0B5ptY5tUIebjdFNoNE5SZVV6LXc",
    "thumbnailLink": "https://lh3.googleusercontent.com/TuQvcHBqKN5oaVCiP71NLXY0tNjlqiCYYQvzwWt3D74e3dEav4PdovaoedHM4iYTKPfFWQ=s220"
	}];
	...
	var tools = {
		...
		getCenteredCSSObjForImage: function( width, height ) {
			...
			var thumbWidth = app.settings.thumb.width,
					thumbHeight = app.settings.thumb.height,
			...
			if( width > height){
			 ...
			 console.log('app.settings.thumb.width : ' + app.settings.thumb.width);
			 console.log('thumbWidth : ' + thumbWidth);
			 console.log('shrunkByHeight : ' + shrunkByHeight);
			 console.log('(thumbWidth - (width * shrunkByHeight))  / 2 : ' + (thumbWidth - (width * shrunkByHeight))  / 2  );
			
			 return {
					'width': app.settings.thumb.width + 'px',
					'height': '100%',
					'margin-left': (  (thumbWidth - (width * shrunkByHeight))  / 2 ) + 'px'
			 }
	...
	var App = function(){
	    ...
	    return function() {
				...
				this.renderFilePngForPost = function(fileId) {
					...
					hsp.showCustomPopup('https://pictographr.com/vendors/hootsuite/progress.html', 'Rendering', 50, 50);
					...
					tools.ajax(url, postObj, 'post', function(data) {
						...
						console.log(url);
						app.attachFile(url);
						hsp.closeCustomPopup();

					});	
					
				};
				
				this.attachFile = function( url ) {
			    ...
			    $('.edit_links').click(function(){
			    		...
			    		that.renderFilePngForPost( $(this).attr('fileId') );
			    ...
			    $('._showModal').click(function(){
			       hsp.showCustomPopup('https://pictographr.com/vendors/hootsuite/modal.html', 'Pictographr', 800, 600);
			    });

			    $('.attach_links').click(function(){
			    	
							var thumbLink = $(this).attr('thumbLink');
							
							that.attachFile(thumbLink);
			    ...
			    $('.thumbs').load(function() {
					  ...
					  $(this).css( tools.getCenteredCSSObjForImage( width, height ));
							...
							<div  class="oh boxes">\
								<div  class="img_wrapper">\
									<div  class="action_links_wrapper">\
										<div>\
											<span class="attach_links action_links icon-app-dir x-attach" thumbLink="' +  thumbLink + '" fileId="' + fileId + '" src="" /></span>\
										</div>\
										<div>\
											<span class="edit_links action_links icon-app-dir x-edit" fileId="' + fileId + '" src="" /></span>\
										</div>\
									</div>\
									<img  class="thumbs" fileId="' + fileId + '" src="' +  thumbLink + '" />\
								</div>\
							</div>\
						';
