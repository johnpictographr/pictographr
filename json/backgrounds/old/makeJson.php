...
$url = 'http://localhost/pictographr/json/backgrounds/imgs/';
$html = file_get_html($url);


foreach($html->find('imgs') as $element)  {
	$path = $url;
	$link = $element->src;
	$url = $path.explode(".", $link)[0].".*";
	$arr['src'] = $url;
	$allArr[] = $arr;
}

echo '<pre>';print_r(  $allArr );echo '</pre>';  
//file_put_contents("backgrounds.json", json_encode($allArr));
