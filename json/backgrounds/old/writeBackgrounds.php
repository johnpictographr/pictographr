...
foreach($html->find('.patternpreview') as $element)  {
	$path = "http://subtlepatterns.com/";
	$style = $element->style;
	$array = explode("url(/", $style);
	if(isset($array[1])){
		$backgroundUrl = $path . $array[1];
		$backgroundUrl = rtrim($backgroundUrl, ')');
		
		if( !isset($allArr[$backgroundUrl])){
			$allArr[$backgroundUrl] = array();
		}
		
		if( !isset($allArr[$backgroundUrl]['tags'])){
			$allArr[$backgroundUrl]['tags'] = array();
		}
		
		$allArr[$backgroundUrl]['tags'][$_GET['tag']] = '';

	}
}

echo '<pre>';print_r( $allArr );echo '</pre>'; 


file_put_contents("backgroundsRaw.json", json_encode($allArr));
